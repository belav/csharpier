...
namespace Worker
{
    public class TypescriptConverter
    {
        ...
        public void DoWork()
        {
            ...
            var typescriptTypeDirectory = Path.Combine(
                rootDirectory.FullName,
                @"prettier-plugin-csharpier\src\Printer");
            var inputFile = Path.Combine(
                typescriptTypeDirectory,
                "PrintAttributeLists.ts");
        ...
        {{");
            ...
            while ((line = streamReader.ReadLine()) != null)
            {
                ...
                line = Regex.Replace(
                    line,
                    @"path.map\(print, ""(\w+)""\)",
                    "node.$1.Select(this.Print)");
                line = Regex.Replace(
                    line,
                    @"path.call\(o => print(\w+)\(o, options, print\), ""(\w+)""\)",
                    "this.Print$1Syntax(node.$2)");
                line = Regex.Replace(
                    line,
                    @"path.map\(\w+ => print(\w+)\(\w+, options, print\), ""(\w+)""\)",
                    "node.$2.Select(this.Print$1Syntax)");
                line = Regex.Replace(
                    line,
                    @"printPathSyntaxToken\(path, ""(\w+)""\)",
                    "String(node.$1.Text)");
                line = Regex.Replace(
                    line,
                    @"node\.(\w+)",
                    o => "node." + o.Groups[1].Value[0].ToString().ToUpper() + o.Groups[
                        1
                    ].Value.Substring(1));
            ...
            var csharpDirectory = Path.Combine(
                rootDirectory.FullName,
                @"CSharpier\CSharpier\Printer");
            var newFile = Path.Combine(
                csharpDirectory,
                nodeNameWithSyntax + ".cs");
