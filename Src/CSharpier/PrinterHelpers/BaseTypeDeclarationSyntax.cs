  ...
  private Doc PrintBaseTypeDeclarationSyntax(BaseTypeDeclarationSyntax node)
  {
    ...
    if (node is TypeDeclarationSyntax typeDeclarationSyntax)
    {
      ...
      if (typeDeclarationSyntax.Members.Count > 0)
      {
        members = Docs.Indent(
          Docs.HardLine,
          Join(Docs.HardLine, typeDeclarationSyntax.Members.Select(this.Print))
        );
      }
    ...
    {
      docs.Add(
        this.PrintSyntaxToken(keyword.Value, afterTokenIfNoTrailing: " ")
      );
    }
    ...
    if (hasMembers)
    {
      ...
      docs.Add(
        groupId != null ? Docs.IfBreak(" ", Docs.Line, groupId) : Docs.HardLine,
