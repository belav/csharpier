...
public class CommandLineFormatter
{
  ...
  private async Task FormatFile(
    ...
    var fileReaderResult = await FileReader.ReadFile(file, cancellationToken);
    ...
    catch (Exception ex)
    {
      ...
      Console.WriteLine(GetPath(file) + " - threw exception while formatting");
    ...
    if (this.Validate)
    {
      ...
      {
        var failure =
          await syntaxNodeComparer.CompareSourceAsync(cancellationToken);
        if (!string.IsNullOrEmpty(failure))
        {
          Interlocked.Increment(ref this.FailedSyntaxTreeValidation);
          Console.WriteLine(GetPath(file) + " - failed syntax tree validation");
  ...
  private bool IgnoreFile(string filePath)
  {
    ...
    if (this.ConfigurationOptions.Exclude.Contains(normalizedFilePath))
    {
      return true;
    }
