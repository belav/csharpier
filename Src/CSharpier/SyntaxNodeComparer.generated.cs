...
public partial class SyntaxNodeComparer
{
  ...
  private CompareResult CompareIdentifierNameSyntax(
    ...
    if (originalNode.IsMissing != formattedNode.IsMissing)
      return NotEqual(originalNode, formattedNode);
    if (originalNode.IsStructuredTrivia != formattedNode.IsStructuredTrivia)
      return NotEqual(originalNode, formattedNode);
    if (originalNode.HasStructuredTrivia != formattedNode.HasStructuredTrivia)
      return NotEqual(originalNode, formattedNode);
    if (originalNode.ContainsSkippedText != formattedNode.ContainsSkippedText)
      return NotEqual(originalNode, formattedNode);
    if (originalNode.ContainsDirectives != formattedNode.ContainsDirectives)
      return NotEqual(originalNode, formattedNode);
    if (originalNode.ContainsDiagnostics != formattedNode.ContainsDiagnostics)
      return NotEqual(originalNode, formattedNode);
    if (originalNode.ContainsAnnotations != formattedNode.ContainsAnnotations)
      return NotEqual(originalNode, formattedNode);
    return Equal;
  }
