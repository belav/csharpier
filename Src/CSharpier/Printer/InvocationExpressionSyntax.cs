...
public partial class Printer
{
  private Doc PrintInvocationExpressionSyntax(InvocationExpressionSyntax node)
  {
    ...
    void Traverse(ExpressionSyntax expression)
    {
      ...
      if (expression is InvocationExpressionSyntax invocationExpressionSyntax)
      {
        ...
        printedNodes.Add(
          new PrintedNode(
            ...
            Doc: Docs.Concat(
              this.PrintSyntaxToken(memberAccessExpressionSyntax.OperatorToken),
      ...
      {
        printedNodes.Add(
          new PrintedNode(Node: expression, Doc: this.Print(expression))
        );
      }
    }
  ...
  private Doc PrintIndentedGroup(IEnumerable<List<Doc>> groups)
  {
    ...
    return Docs.Indent(
      Docs.Group(
        Join(Docs.SoftLine, groups.Select(o => Docs.Group(o.ToArray())))
      )
    );
  }
}
