namespace Insite.Spire.Services
{
    ...
    public class PageLinkService : IPageLinkService
    {
        ...
        public IList<PageLinkModel> GetLinks(IUnitOfWork unitOfWork, ISiteContext siteContext)
        {
            ...
            if (!this.cacheManager.Contains(checkedPageUrlsCacheKey))
            {
                if (!unitOfWork
                    ...
                    .Any(o => o.LanguageId == siteContext.LanguageDto.Id && o.WebsiteId == siteContext.WebsiteDto.Id))
                {
                    var homePageNodes = unitOfWork
                        .GetRepository<Node>()
                        .GetTableAsNoTracking()
                        .Where(o => o.Type == "HomePage" && o.WebsiteId == siteContext.WebsiteDto.Id)
                        .Select(o => o.Id)
                        .ToList();
            ...
            var queryable = unitOfWork.GetRepository<PageUrl>().GetTableAsNoTracking()
                .Where(
                    o => o.LanguageId == siteContext.LanguageDto.Id
                         && o.WebsiteId == siteContext.WebsiteDto.Id);
            var pageUrls = queryable.Where(
                    o => o.PublishOn.HasValue && o.PublishOn < DateTimeOffset.Now
                                              && (o.PublishUntil == null || o.PublishUntil > DateTimeOffset.Now))
                .Select(
            ...
            {
                unpublishedPageUrls = queryable.Where(o => !o.PublishOn.HasValue)
                    .Select(
                        o => new PageUrlSelector
                        {
                            NodeId = o.NodeId,
            ...
            var getSignInPageUrl = new Lazy<string>(() => RetrievePageService.GetSignInPageUrl(this.retrievePageService, unitOfWork, siteContext));
            foreach (var thePageUrl in pageUrls)
            {
                if (unpublishedPageUrls == null || !unpublishedPageUrls.TryGetValue(thePageUrl.NodeId, out var pageUrl))
                {
                    pageUrl = thePageUrl;
                }
                ...
                var navigationFilterResult = this.navigationFilterService.ApplyFilters(pageUrl.Type, parameter);
            ...
            void LoadChildren(IEnumerable<PageLinkModel> pageLinks)
            {
                foreach (var pageLink in pageLinks)
                {
                    ...
                    pageLink.Children = allLinks[pageLink.Id].GroupBy(o => o.Id).Select(o => o.OrderBy(x => x.PublishOn).First()).OrderBy(o => o.SortOrder).ToList();
    ...
    {
        IList<PageLinkModel> GetLinks(IUnitOfWork unitOfWork, ISiteContext siteContext);
    }
}
